...
def index():
    """
    Root route. Serves the client HTML file (assumed to be index.html) 
    so the user can see the game interface.
    """
    # Assuming the client file (e.g., v3.html) has been renamed to index.html
    # and placed in the same directory as the server file.
    try:
        return send_file('index.html') # Serve the client HTML file
    except FileNotFoundError:
        return "Blackjack Server is running, but the client file (index.html) was not found. Please ensure your client HTML is named index.html and deployed in the root directory.", 404
